...
from utils.fast_rcnn_utils import clone_model, clone_conv_layers, create_fast_rcnn_predictor, \
...
def prepare(cfg, use_arg_parser=True):
    ...
    cfg['MODEL_PATH'] = os.path.join(cfg.OUTPUT_PATH, cfg["DATA"].DATASET,"faster_rcnn_eval_{}_{}.model"
                                     .format(cfg["MODEL"].BASE_MODEL, "e2e" if cfg["CNTK"].TRAIN_E2E else "4stage"))
    cfg['BASE_MODEL_PATH'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), "model",
