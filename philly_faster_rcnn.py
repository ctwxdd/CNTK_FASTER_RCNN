...
from cntk.cntk_py import binary_cross_entropy, start_profiler, stop_profiler, set_computation_network_trace_level
...
def train_model(network, trainer, od_minibatch_source, reader_dev, minibatch_size, epochs_to_train, restore, cfg, profiling=False):
...
def faster_rcnn(max_epochs=None, restore=False, log_to_file=None, minibatch_size= None,
            num_mbs_per_log=None, block_size=3200, gen_heartbeat=False, profiling=False, tensorboard_logdir=None, 
            l1_regularization_weight=0, l2_regularization_weight=0):

    #set_computation_network_trace_level(1)

    #read and parse the cong file
    cfg = get_configuration()
    prepare(cfg, False)
    ##over write the parameters

    if max_epochs:
        cfg["CNTK"].E2E_MAX_EPOCHS = max_epochs 
    if minibatch_size: 
        cfg.MB_SIZE = minibatch_size
    ...
    eval_model = train_model(network, trainer, reader_train, None, cfg.MB_SIZE, cfg["CNTK"].E2E_MAX_EPOCHS, restore, cfg, profiling) 
